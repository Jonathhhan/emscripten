...
.. c:macro:: EMSCRIPTEN_KEEPALIVE
  ...
  defined is otherwise included by the linker.  This occurs when the object file
  is part of an archive, and is not otherwise referenced. In this case any
  symbols in the object file not be included or exported.  One way to work
  around this limitation is to use the ``-Wl,--whole-archive`` /
